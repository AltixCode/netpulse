...
function withIosSceneLifecycle(config) {
  ...
  config = withAppDelegate(config, (config) => {
    if (config.modResults.language !== 'swift') {
      throw new Error('with-ios-scene-lifecycle requires a Swift AppDelegate.');
    }

    const legacyWindowStartup = /#if os\(iOS\) \|\| os\(tvOS\)\n\s*window = UIWindow\(frame: UIScreen\.main\.bounds\)\n\s*factory\.startReactNative\([\s\S]*?launchOptions: launchOptions\)\n#endif/;
    if (!legacyWindowStartup.test(config.modResults.contents)) {
      throw new Error('Unable to locate the Expo legacy window startup block.');
    }
    config.modResults.contents = config.modResults.contents.replace(
      legacyWindowStartup,
      '// React Native UI startup is owned by SceneDelegate on iOS.'
    );
    return config;
  });
  ...
  config = withXcodeProject(config, (config) => {
    ...
    const sceneDelegatePath = path.join(platformProjectRoot, relativePath);
    fs.writeFileSync(sceneDelegatePath, SCENE_DELEGATE);

    if (!config.modResults.hasFile(sceneDelegatePath)) {
