...
const TRACKING_PURPOSE =
  'This lets NetPulse show ads that are more relevant to you. Your measurements are never uploaded, your data is never sold, and the app works exactly the same either way.';
...
export default ({ config }: ConfigContext): ExpoConfig => {
  ...
  return {
    ...config,
    name: config.name ?? 'NetPulse',
    slug: config.slug ?? 'netpulse',
    plugins: [
