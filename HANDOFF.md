...
| | |
|---|---|
| Bundle id / package | `com.altixcode.netpulse` |
| App Store Connect app | `6811548308` (version 1.0, `PREPARE_FOR_SUBMISSION`) |
| In-app purchase | `netpulse_pro_lifetime` — ASC id `6811578373`, state `MISSING_METADATA` |
| RevenueCat project | `projf99e20eb` |
| RevenueCat apps | iOS `app277c9444d5` · Android `appea131e5496` |
...
- **Interstitial**: app/index.tsx — at the START of the next test, never over results. Pacing lives in `adPolicy.ts`: the first completion
...
1. **Fix the entitlement mismatch — this one ships broken.**
   ...
   rc entitlements create --lookup-key remove_ads \
     --display-name "Pro — Ad-Free & Unlimited" --project-id projf99e20eb --json --yes
   rc entitlements attach <entitlementId> <productId> --project-id projf99e20eb --json --yes
   ```
...
4. **Upload the IAP review screenshot.** The in-app purchase
   (`netpulse_pro_lifetime`) is localized and priced at $3.99, and sits at
   `MISSING_METADATA` for want of one screenshot:

   ```bash
   asccli iap-review-screenshot upload --iap-id 6811578373 --file <path-to-png>
   ```
...
2. **RevenueCat — Apple credentials.** `app277c9444d5` (the App Store app in project
   `projf99e20eb`) has no Apple credentials, so App Store purchases cannot be
   validated. This needs an interactive Apple ID sign-in with 2FA:

   ```bash
   rc setup apple app277c9444d5
   ```
...
3. **GitHub secrets.** The signing secrets are all present
   ...
   Set each with `gh secret set <KEY> --repo AltixCode/netpulse`. The RevenueCat
   public SDK keys are fetchable — `rc api GET "/projects/projf99e20eb/apps/app277c9444d5/public_api_keys"`
   — the AdMob ones come from step 1.

4. **The legal URLs are wrong and they 404.** `src/config/legal.ts` points at
   `https://www.hushtunnel.com/legal/netpulse-privacy` — **hushtunnel.com**, a leftover from the HushTunnel
...
5. **Play Console — the app record exists.** `com.altixcode.netpulse` is recognised,
...
- **The home screen claimed "100% Ad-Free Network Suite"** and "without
  commercial ads" — above a banner. Corrected in every locale, along with the
  paywall's "100% Ad-Free & Private" title.
- **The German, French, Spanish and Italian store descriptions** promised
  "ohne Werbung und ohne Tracking" and equivalents. Corrected in
  `Dev/scripts/store-metadata.json`, which now also discloses AdMob in the
  PRIVACY paragraph of every locale.
...
../scripts/verify-app.sh netpulse com.altixcode.netpulse   # full build + device verification, both platforms
```
